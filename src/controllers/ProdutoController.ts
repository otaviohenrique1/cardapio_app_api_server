...
import { valida_alualizacao_produto, valida_criacao_produto } from "../utils/SchemasValidacao";
...
/**
 * Listar todas os produtos cadastradas pelo usuario, usando o id do mesmo e se o valor da coluna 'ativo' for true
 */
 export async function listar_produtos_ativos(request: Request, response: Response, next: NextFunction) {
  const { id } = request.params;
  const produtoRepository = getRepository(Produto);
  const produto = await produtoRepository.find({
    where: { 
...
/**
 * Busca um produto cadastrada usando o id da mesma e exibe os seus dados
 */
...
/**
 * Apaga um produto, usando o id da mesmo
 */
...
/**
 * Atualiza os dados de um produto, usando o id da mesma para busca-la no banco de dados
 */
